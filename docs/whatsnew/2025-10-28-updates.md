...
If you're building an application that uses AntGent as a dependency, you can easily integrate the summarization capabilities by adding the API endpoints and configuring your Temporal workers.

### Enable Summarizer API Endpoints

Add the AntGent summarizer router to your application's server configuration:

```python
# In your_app/server/server.py

class YourAppServer(Server):
    _routers: ClassVar[set[str]] = {
        "your_app.server.api.some_endpoint:router",
        "your_app.server.api.another_endpoint:router",
        # Add the AntGent summarizer router
        "antgent.server.api.workflows.summarizer:router",
        # Optional: Add generic status endpoint
        "antgent.server.api.status:router",
    }
```

This exposes all summarizer endpoints at `/api/workflows/summarizer*` in your application.

### Configure Temporal Workers

Your Temporal workers need to be configured to execute the summarizer workflows and activities. Add these paths to your worker definitions:

```yaml
# localconfig.yaml
temporalio:
  workers:
    - name: "your-app-workflow-worker"
      queue: "your-app-queue"
      workflows:
        # Your existing workflows
        - "your_app.workflows.some_workflow:SomeWorkflow"
        # Add AntGent summarizer workflows
        - "antgent.workflows.summarizer:TextSummarizerOneTypeWorkflow"
        - "antgent.workflows.summarizer:TextSummarizerAllWorkflow"

    - name: "your-app-activity-worker"
      queue: "your-app-activity-queue"
      activities:
        # Your existing activities
        - "your_app.workflows.some_workflow:some_activity"
        # Add AntGent summarizer activity
        - "antgent.workflows.summarizer:run_summarizer_one_type_activity"
```

### Configure LLM Access

Ensure your configuration includes the necessary LLM provider settings:

```yaml
# localconfig.yaml
llms:
  litellm:
    api_key: "${LITELLM_API_KEY}"  # Or your provider's key
...
After these changes, restart your server and workers. The summarizer endpoints will be available in your application's API.
